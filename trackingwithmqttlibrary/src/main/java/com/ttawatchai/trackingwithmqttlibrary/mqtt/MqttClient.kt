package com.ttawatchai.trackingwithmqttlibrary.mqtt
...
class MqttClien(private val context: Context ,config: String) {
    ...
    fun publishMessage(config: String, steam: String): Boolean {
        try {
    ...
    fun setClien(config: String): Boolean {
        val gson = Gson()
        lateinit var data: MqttResponse
        try {
            data = gson.fromJson(config, MqttResponse::class.java)
